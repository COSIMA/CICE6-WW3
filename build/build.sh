...
if [ "$ACTIVE_OCN" = true ] ; then
    ...
    if [ -L ${bld_dir}"/ocn/obj" ]; then
        unlink ${bld_dir}"/ocn/obj"
    else
        rm -r ${bld_dir}"/ocn/obj"
    fi
    ln -sf ${sharedlib_dir}/CDEPS/docn ${bld_dir}/ocn/obj
    cd ${bld_dir}/ocn/obj
    make docn
    ln -sf ${bld_dir}/ocn/obj/libdocn.a ${bld_dir}/lib/libocn.a
fi
cd ${cwd}
